...
struct IndexOptions: ParsableArguments {
    ...
    func run(kind: IndexScope.Kind, directory: String?, name: String, rebuild: Bool = false,
        ...
        do {
            // Retain CLI schema validation before the shared native evaluator loads definitions.
            if resolvedConfigPath.exists { _ = try MdUtilsConfig.load(from: resolvedConfigPath, projectRoot: root) }
