...
Rules match Markdown files by project-relative glob patterns, optional frontmatter conditions, and optional document conditions. Files can match more than one rule, in which case every matching check applies.
...
Supported document matcher operators are `hasHeading`, which matches an exact ATX Markdown heading text in the document body.
