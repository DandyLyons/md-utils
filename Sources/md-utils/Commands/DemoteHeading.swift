...
extension CLIEntry {
  ...
  struct DemoteHeading: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
      commandName: "demote",
      abstract: "Demote headings (increase level) in Markdown files",
      discussion: """
        Demotes (increases level) a heading and optionally its children.
        For example, H1 → H2, H2 → H3.
    ...
    @Flag(
      name: .long,
      help: "Demote only the target heading, not its children"
    )
    ...
    private func processFile(_ path: Path, totalFiles: Int) async throws {
      ...
      do {
        result = try await doc.demoteHeading(at: index, includeChildren: !targetOnly)
      ...
      if inPlace {
        ...
        if totalFiles > 1 {
          FileHandle.standardError.write("Demoted heading in: \(path)\n".data(using: .utf8)!)
        }
      } else {
        // Output to stdout
        if totalFiles > 1 {
