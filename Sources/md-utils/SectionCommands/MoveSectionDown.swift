...
extension CLIEntry {
  /// Move a section down by one position among its siblings.
  struct MoveSectionDown: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
      commandName: "move-down",
      abstract: "Move a section down among its siblings",
      discussion: """
        Moves a section (heading + all nested content) down among its sibling headings.
        ...
        EXAMPLES:
          # Move first heading down by one
          md-utils section move-down --index 1 document.md

          # Move by name, down by 2 positions
          md-utils section move-down --name "Introduction" --count 2 document.md

          # Move in place
          md-utils section move-down --name "Setup" --in-place document.md
    ...
    private func processFile(_ path: Path) async throws {
      ...
      do {
        if let idx = index {
          result = try await doc.moveSectionDown(at: idx, count: count)
        } else if let headingName = name {
          result = try await doc.moveSectionDown(byName: headingName, caseSensitive: caseSensitive, count: count)
        } else {
