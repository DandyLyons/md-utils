...
public struct EndpointPlanCompiler: Sendable {
  ...
  public func compile(_ configuration: MarkdownServerConfiguration) throws -> EndpointPlan {
    ...
    if !["1", "2", "3"].contains(configuration.serverConfigVersion) {
      diagnostics.append(EndpointPlanDiagnostic(
        code: .unsupportedConfigurationVersion,
        location: "serverConfigVersion",
        message: "Unsupported server configuration version \"\(configuration.serverConfigVersion)\"; expected 1, 2, or 3"
      ))
    }
    ...
    for (index, resource) in configuration.resources.enumerated() {
      ...
      if resource.operations.isEmpty && resource.mutations?.operations.isEmpty != false {
        diagnostics.append(EndpointPlanDiagnostic(
      ...
      if let mutations = resource.mutations {
        var reasons: [String] = []
        if configuration.serverConfigVersion != "3" { reasons.append("Mutations require serverConfigVersion 3.") }
        if resource.writable == nil { reasons.append("Mutations require an explicit writable codec.") }
        if mutations.operations.isEmpty || Set(mutations.operations).count != mutations.operations.count {
          reasons.append("Mutation operations must be nonempty and unique.")
        }
        if mutations.idempotencyRetentionSeconds < 1 { reasons.append("Retention must be positive.") }
        if mutations.operations.contains(.identity) && mutations.identityFields.isEmpty {
          reasons.append("Identity editing requires explicit identityFields.")
        }
        ...
        if let persistent = configuration.persistentIdentity?.path.first,
          mutations.identityFields.contains(persistent) || mutations.creation?.identifiers.contains(persistent) == true {
          reasons.append("Persistent UUID is generated by the host and changed only by UUID repair.")
        }
        if mutations.operations.contains(.repairUUID) && configuration.persistentIdentity == nil {
          reasons.append("UUID repair requires persistentIdentity.")
        }
        if let slug = mutations.creation?.slug,
          resource.writable?.codec.frontmatterFields.contains(slug.field) != true && !protected.contains(slug.field) {
          reasons.append("Generated slug must be a writable or protected field.")
        }
        for reason in reasons {
          diagnostics.append(.init(code: .invalidLookup, location: "\(location).mutations", message: reason))
        }
        for operation in mutations.operations {
          ...
          routeCandidates.append(RouteCandidate(description: .init(method: operation.method,
            path: .init(validated: resource.route + suffix), kind: .mutation,
            resourceName: resource.name, operationID: id, mutationOperation: operation,
          ), location: "\(location).mutations"))
          operationIDCandidates.append(.init(id: id, location: "\(location).mutations"))
          if operation != .create {
            for lookup in resource.lookups {
              for query in [true, false] where query || lookup.source != .logicalPath {
                ...
                routeCandidates.append(RouteCandidate(description: .init(method: operation.method,
                  ...
                  kind: .mutation, resourceName: resource.name, operationID: lookupID,
                  lookupName: lookup.name, mutationOperation: operation, lookupUsesQuery: query,
                ), location: "\(location).mutations"))
                operationIDCandidates.append(.init(id: lookupID, location: "\(location).mutations"))
              }
            }
          }
          // Exact path addressing is resource scoped and never bypasses selection.
          if operation != .create {
            let pathID = id + ".path"
            routeCandidates.append(RouteCandidate(description: .init(method: operation.method,
              path: .init(validated: resource.route + "/_mutations/" + operation.rawValue + "/by-path"),
              kind: .mutation, resourceName: resource.name, operationID: pathID,
              mutationOperation: operation, lookupUsesQuery: true,
            ), location: "\(location).mutations"))
            operationIDCandidates.append(.init(id: pathID, location: "\(location).mutations"))
          }
        }
        let statusID = "\(resource.name).mutationStatus"
        routeCandidates.append(RouteCandidate(description: .init(method: .get,
          path: .init(validated: resource.route + "/_operations/{id}"), kind: .mutationStatus,
          resourceName: resource.name, operationID: statusID,
        ), location: "\(location).mutations"))
        operationIDCandidates.append(.init(id: statusID, location: "\(location).mutations"))
        routeCandidates.append(RouteCandidate(description: .init(method: .post,
          path: .init(validated: resource.route + "/_operations/{id}/resolve"), kind: .mutationRecovery,
          resourceName: resource.name, operationID: statusID + ".resolve",
        ), location: "\(location).mutations"))
        operationIDCandidates.append(.init(id: statusID + ".resolve", location: "\(location).mutations"))
      }
