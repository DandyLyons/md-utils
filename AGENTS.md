...
- **Knap templates**: `MarkdownUtilitiesTemplates` uses SwiftKnap through its public Swift API, outside Core/WASM. `template render` combines a Knap body with explicit JSON frontmatter serialized by Yams. Schema/document validation, bounded output, and Knap diagnostics are shared by CLI and server creation. SwiftKnap owns execution and portability. Linux builds require its documented native development/runtime packages; see `docs/template-rendering.md`. Tests: `swift test --filter 'MarkdownUtilitiesTemplatesTests|TemplateTests|MarkdownMutationTests'`.
