...
struct CLIEntryTests {
  ...
  func `CLI configuration is properly set`() async throws {
    ...
    #expect(config.subcommands[5] is CLIEntry.FormatCommand.Type)
    #expect(config.subcommands[6] is CLIEntry.FrontMatterCommands.Type)
    #expect(config.subcommands[7] is CLIEntry.GenerateTOC.Type)
