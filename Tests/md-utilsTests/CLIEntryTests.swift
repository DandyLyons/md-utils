...
struct CLIEntryTests {
  ...
  func `CLI configuration is properly set`() async throws {
    ...
    #expect(config.subcommands[0] is CLIEntry.ConvertCommands.Type)
    #expect(config.subcommands[1] is CLIEntry.DemoteHeading.Type)
    ...
    #expect(config.subcommands[6] is CLIEntry.Lines.Type)
    #expect(config.subcommands[7] is CLIEntry.PromoteHeading.Type)
  }
}
