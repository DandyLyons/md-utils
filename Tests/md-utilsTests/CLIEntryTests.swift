...
struct CLIEntryTests {
  ...
  func `CLI configuration is properly set`() async throws {
    ...
    #expect(config.subcommands[7] is CLIEntry.LinkCommands.Type)
    #expect(config.subcommands[8] is CLIEntry.Lines.Type)
    #expect(config.subcommands[9] is CLIEntry.SectionCommands.Type)
  }
}
