...
struct CLIEntryTests {
  ...
  func `CLI configuration is properly set`() async throws {
    ...
    #expect(config.subcommands.count == 11)
    ...
    #expect(config.subcommands[4] is CLIEntry.FormatCommand.Type)
    #expect(config.subcommands[5] is CLIEntry.FrontMatterCommands.Type)
    #expect(config.subcommands[6] is CLIEntry.GenerateTOC.Type)
    #expect(config.subcommands[7] is CLIEntry.HeadingCommands.Type)
    #expect(config.subcommands[8] is CLIEntry.LinkCommands.Type)
    #expect(config.subcommands[9] is CLIEntry.Lines.Type)
    #expect(config.subcommands[10] is CLIEntry.SectionCommands.Type)
  }
}
